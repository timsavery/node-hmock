...
function MockedRequest(options, expectations, callback) {
	...
	this.end = function() {
		...
		if (e) {
			...
			var expectedResponse = e.getResponse(),
					res = new MockedResponse(expectedResponse.getHeaders());

			res.statusCode = expectedResponse.getStatusCode();

			callback(res);

			res.emit('data', JSON.stringify(expectedResponse.getBody()));
			res.emit('end');
		}
	};
