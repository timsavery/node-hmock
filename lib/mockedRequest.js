...
function MockedRequest(options, expectation, callback) {
	...
	this.end = function() {
		...
		var eResponse = expectation.getResponse(),
				mResponse = new MockedResponse(eResponse.getHeaders());

		mResponse.statusCode = eResponse.getStatusCode();

		callback(mResponse);

		mResponse.emit('data', JSON.stringify(eResponse.getBody()));
		mResponse.emit('end');
	};
	...
	function verifyExpectation() {
		...
		if (typeof eBody === 'string') {
			if (!new RegExp(eBody, 'i').test(aBody)) {
